...
typedef struct hash_table_node_s ht_node;
typedef struct hash_table_s ht;

struct adjacency_node_s
{
  adjacency_node_t *next; // link to th enext adjacency list node
  ht_node *vertex;        // the other vertex
};
...
struct hash_table_node_s
{
  ...
  ht_node *next;              // next hash table linked list node
  // the vertex data
  ...
  ht_node *previous;      // breadth-first search parent
  // the union find data
  ht_node *representative; // the representative of the connected component this vertex belongs to
...
struct hash_table_s
{
  ...
  ht_node **heads;                // the heads of the linked lists
};
...
static ht_node *allocate_hash_table_node(void)
{
  ht_node *node;
...
static void free_hash_table_node(ht_node *node)
{
  free(node);
}
...
static ht_node *hash_table_create(void)
{
  ht *table;
  unsigned int i;
  ...
  table->number_of_entries = 0u;
  table->hash_table_size = 256u;
  table->heads = calloc(table->hash_table_size, sizeof(ht));
  if (table->number_of_entries == NULL)
  {
...
static bool hash_table_grow(ht *table)
{
  // Allocate new entries array.
  size_t new_capacity = table->hash_table_size * 2;
  if (new_capacity < table->hash_table_size)
  {
    return false; // overflow (capacity would be too big)
  }
  ht_node *new_entries = calloc(new_capacity, sizeof(ht_node));
  if (new_entries == NULL)
  {
    return false;
  }
  ...
  for (size_t i = 0; i < table->hash_table_size; i++)
  {
    ht_node *entry = table->heads[i];
    if (entry->word != NULL)
    {
      ht_set_entry(new_entries, new_capacity, entry->word,
                   entry.value, NULL);
    }
  }
  ...
  free(table->heads);
  table->heads = new_entries;
  table->hash_table_size = new_capacity;
  return true;
}

static void hash_table_free(ht *hash_table)
{
  //
  // complete this
  //
...
static ht_node *find_word(ht *hash_table, const char *word, int insert_if_not_found)
{
  ht_node *node;
  unsigned int i;
...
static ht_node *find_representative(ht_node *node)
{
  ht_node *representative, *next_node;
...
static void add_edge(ht *hash_table, ht_node *from, const char *word)
{
  ht_node *to, *from_representative, *to_representative;
...
static void similar_words(ht *hash_table, ht_node *from)
{
...
static int breadth_first_search(int maximum_number_of_vertices, ht_node **list_of_vertices, ht_node *origin, ht_node *goal)
{
  //
  // complete this
  //
...
static void list_connected_component(ht *hash_table, const char *word)
{
  //
  // complete this
  //
...
static ht_node **largest_diameter_example;

static int connected_component_diameter(ht_node *node)
{
  int diameter;
...
static void path_finder(ht *hash_table, const char *from_word, const char *to_word)
{
  //
  // complete this
  //
...
static void graph_info(ht *hash_table)
{
  //
  // complete this
  //
